...
# ① 把函数放到文件顶部或类里皆可
def compute_predicted_return(agent_signals: list) -> float:
    ...
    # （2）信号→数值映射
    mapping = {"bullish": 0.06, "neutral": 0.00, "bearish": -0.06}

    # （3）累加加权收益
    est = 0.0
    for s in agent_signals:
        # 兼容两种键名
        name   = s.get("agent_name") or s.get("agent") or ""
        signal = s.get("signal", "neutral")
        est   += weights.get(name, 0) * mapping.get(signal, 0)

    # （4）返回百分比并保留两位小数
    return round(est * 100, 2)

    # """按权重把看多/看空/中性映射为收益率 (%)"""
    # weights = {"valuation_analysis": 0.35,
    #            "fundamental_analysis": 0.30,
    #            "technical_analysis": 0.25,
    #            "sentiment_analysis": 0.10}
    # mapping = {"bullish": 0.06, "neutral": 0.00, "bearish": -0.06}
    # est = sum(weights.get(s["agent"], 0) *
    #           mapping.get(s["signal"], 0) for s in agent_signals)
    # return round(est * 100, 2)          # → 百分比并保留两位小数
...
def portfolio_management_agent(state: AgentState):
    ...
    user_message = {
        ...
        "content": f"""Based on the team's analysis below, make your trading decision.
            ...
            Only include the action, quantity, predicted_return_pct, reasoning, confidence, and agent_signals in your output as JSON.  Do not include any JSON markdown.
    ...
    try:
        ...
        decision = json.loads(re.sub(r'```(?:json)?|```', '', result).strip())

# 如果模型没填 predicted_return_pct，就自己算
    if ("predicted_return_pct" not in decision or decision["predicted_return_pct"] == 0):
        decision["predicted_return_pct"] = compute_predicted_return(decision["agent_signals"])
    else:
        p = decision["predicted_return_pct"]
        if abs(p) < 1:                       # 把 -0.05 → -5.00 %
            decision["predicted_return_pct"] = round(p * 100, 2)

    result = json.dumps(decision, ensure_ascii=False)
    ...
    if result is None:
        result = json.dumps({
            ...
            "quantity": 0,
            "confidence": 0.7,
            "predicted_return_pct": compute_predicted_return([
                {"agent_name": "Valuation Analysis",   "signal": "bearish"},
                {"agent_name": "Fundamental Analysis", "signal": "bullish"},
                {"agent_name": "Technical Analysis",   "signal": "neutral"},
                {"agent_name": "Sentiment Analysis",   "signal": "bullish"},
            ]),
            "agent_signals": [
                {
