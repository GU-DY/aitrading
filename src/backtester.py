...
class Backtester:
    ...
    def run_backtest(self):
        ...
        for current_date in dates:
            ...
            predicted_pct = agent_decision.get("predicted_return_pct")
            if predicted_pct is None:
                if action == "buy":
                    ...
                    predicted_pct = 0
            self.predicted_changes.append({"Date": current_date, "Predicted Change": predicted_pct})
    ...
    # 获取完整历史价格数据
        ...
        price_df["Actual Change"] = price_df["close"].pct_change() * 100
        pred_df = pd.DataFrame(self.predicted_changes).set_index("Date")
        pred_df.index = pred_df.index.normalize()
        performance_df = pd.DataFrame(self.portfolio_values).set_index("Date")
        performance_df.index = performance_df.index.normalize()

        merged_df = price_df.join(performance_df, how='inner')
        merged_df = merged_df.join(pred_df, how='left')
        merged_df = merged_df[~merged_df.index.duplicated(keep='first')]


        # 创建专业级图表布局
        plt.figure(figsize=(16, 12))
        gs = plt.GridSpec(4, 1, height_ratios=[3, 1, 1, 0.5])
        ax1 = plt.subplot(gs[0])  # 蜡烛图
        ax2 = plt.subplot(gs[1],sharex=ax1)  # 组合价值
        ax3 = plt.subplot(gs[2],sharex=ax1)  # 累计收益
        ax4 = plt.subplot(gs[3], sharex=ax1)  # 预测与实际涨跌幅
        ...
        # 转换日期格式
        dates = merged_df.index.to_pydatetime()
        opens = merged_df['open'].values
        highs = merged_df['high'].values
        lows = merged_df['low'].values
        closes = merged_df['close'].values
        ...
        for idx in range(len(dates)):
            color = 'r' if closes[idx] > opens[idx] else 'g'
            # plt.setp(ax1, autoscaley_on=False)
            ax1.vlines(dates[idx], lows[idx], highs[idx], color=color, linewidth=1)
            ax1.vlines(dates[idx], min(opens[idx], closes[idx]), 
                    max(opens[idx], closes[idx]), color=color, linewidth=4)

        # 标注交易信号

        for trade in self.trade_history:
            date = pd.to_datetime(trade['date']).normalize()
            if date not in merged_df.index:
                continue
            ...
            if action == 'buy':
                ...
                color = 'maroon'
                y_pos = price + y_offset
            ax1.scatter(date, y_pos,
                marker=marker, s=120, color=color,
                edgecolors='white', linewidths=1, zorder=5)
            ax1.plot(date, y_pos, marker, markersize=12, color=color, 
                    markeredgecolor='white', markeredgewidth=1)

        ax1.set_title(f"{self.ticker} 股价走势与交易信号", fontsize=14, pad=20)
        ax1.grid(True, alpha=0.3)
        plt.setp(ax1.get_xticklabels(), visible=False)

        # 绘制组合价值（对数坐标）
        ax2.semilogy(merged_df.index, merged_df["Portfolio Value"], 
                    color='navy', linewidth=2)
        ax2.fill_between(merged_df.index, merged_df["Portfolio Value"], 
                        alpha=0.3, color='skyblue')
        ax2.set_title("资产组合价值变化", fontsize=12)
        ax2.grid(True, alpha=0.3)
        plt.setp(ax2.get_xticklabels(), visible=False)
        merged_df["Cumulative Return"] = (merged_df["Portfolio Value"] / self.initial_capital - 1) * 100

        # 绘制累计收益率
        ax3.plot(merged_df.index, merged_df["Cumulative Return"], 
        ...
        ax4.plot(
            merged_df.index,
            merged_df["Actual Change"],
            label="实际涨跌幅",
            color="tab:blue",
        )
        ax4.plot(
            merged_df.index,
            merged_df["Predicted Change"],
            label="预测涨跌幅",
            color="tab:red",
            linestyle="--",
        )
        ax4.set_title("预测与实际涨跌幅 (%)", fontsize=12)
        ax4.legend()
        ...
        legend_elements = [
            Line2D([0], [0], color='r', lw=4, label='上涨'),
            Line2D([0], [0], color='g', lw=4, label='下跌'),
            Line2D([0], [0], marker='^', color='darkgreen', label='买入信号',
                markersize=12, markeredgecolor='white'),
            Line2D([0], [0], marker='v', color='maroon', label='卖出信号',
                markersize=12, markeredgecolor='white')
        ]
        ...
        # 添加专业注释
        final_value = merged_df["Portfolio Value"].iloc[-1]
        total_return = (final_value/self.initial_capital-1)*100
        ax2.annotate(f'最终价值: ¥{final_value:,.2f}\n收益率: {total_return:.1f}%',
...
if __name__ == "__main__":
    ...
    parser.add_argument('--ticker', type=str,default='601398',
                        help='股票代码 (例如: 600519)')
    parser.add_argument('--end-date', type=str,
                        default='2025-4-21', help='结束日期，格式：YYYY-MM-DD') #datetime.now().strftime('%Y-%m-%d')
    parser.add_argument('--start-date', type=str, default='2025-4-18', help='开始日期，格式：YYYY-MM-DD')
